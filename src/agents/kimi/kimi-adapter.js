...
export function createKimiAdapter({ permissionPolicy, log, client, createClient, clientOptions, ...options } = {}) {
  return createAcpAdapter({
    ...
    client,
    createClient:
      client ? undefined : createClient ?? ((cwd) => new KimiAcpClient({ permissionPolicy, onLog: log, cwd, ...clientOptions })),
    ...options
  });
}
